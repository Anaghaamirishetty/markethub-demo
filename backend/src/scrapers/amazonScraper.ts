...
export const scrapeAmazonProduct = async (rawUrl: string) => {
  ...
  try {
    ...
    browser = await puppeteer.launch({
      headless: false,
      executablePath:
        "D:\\puppeteer-cache\\chrome\\win64-143.0.7499.40\\chrome-win64\\chrome.exe",
    ...
    const ratingNumber =
      ratingText && ratingText.includes("out")
    ...
    return { error: "Scraping failed", details: err };
  }
};
