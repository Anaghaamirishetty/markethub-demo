...
import { 
  Star, ExternalLink, Bell, Heart, Share2, MapPin, 
  MessageSquare, Send, TrendingDown, TrendingUp, Minus,
...
export default function ProductView() {
  ...
  const { addToTracklist, addAlert, tracklist } = useAppStore();
  ...
  const isTracked = tracklist.some(t => t.productId === productId);
  ...
  const recommendationStyles = {
    ...
    high: { bg: 'bg-destructive', text: 'Wait for price drop', icon: TrendingUp },
  };

  const rec = recommendationStyles[product.buyRecommendation as keyof typeof recommendationStyles];
  ...
  const handleAddToTracklist = () => {
    addToTracklist(product.id);
    toast({ title: 'Added to tracklist!', description: `${product.name} is now being tracked.` });
  };
  ...
  const handleAddAlert = () => {
    ...
    addAlert({
      ...
      status: 'active',
      emailEnabled: emailAlert,
      createdAt: new Date().toISOString(),
    });
    toast({ title: 'Alert created!', description: `We'll notify you when price drops to ₹${targetPrice}` });
    setTargetPrice('');
  };

  const dummyComments = [
    { id: 1, user: 'Rahul S.', text: 'Great product! Got it during sale for even less.', date: '2 days ago' },
    { id: 2, user: 'Priya M.', text: 'Lowest price I found was on Amazon during Prime Day.', date: '1 week ago' },
    { id: 3, user: 'Amit K.', text: 'Quality is excellent. Worth the price.', date: '2 weeks ago' },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-4">
                <div className={`w-4 h-full min-h-[60px] rounded-full ${rec.bg}`} />
                <div>
              ...
              {priceEntries.map((entry, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-medium capitalize">{entry.store}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xl font-bold">₹{entry.price.toLocaleString()}</span>
                    <Button 
                      size="sm" 
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <Label htmlFor="email-alert" className="flex items-center gap-2 cursor-pointer">
                  Send to my email
                </Label>
                <Switch
                  id="email-alert"
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={() => { setComment(''); toast({ title: 'Comment posted!' }); }}>
