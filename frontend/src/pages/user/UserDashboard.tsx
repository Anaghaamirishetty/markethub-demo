...
import { 
  TrendingDown, Star, Bell, Package, Sparkles, 
  ChevronRight, Eye, Clock, ArrowRight
} from 'lucide-react';
...
export default function UserDashboard() {
  ...
  const recentProducts = productsData.products.slice(2, 6);
            ...
            <CardContent className="space-y-3">
              {alerts.slice(0, 3).map((alert) => (
                <div key={alert.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  ...
                  <Badge variant={alert.status === 'active' ? 'default' : 'secondary'} className="ml-2">
                    {alert.status}
                  </Badge>
                </div>
              ))}
            ...
            <CardContent className="space-y-3">
              {notifications.slice(0, 4).map((notif) => (
                <div key={notif.id} className="flex items-start gap-3 p-2">
                  <div className={`w-2 h-2 mt-2 rounded-full ${notif.read ? 'bg-muted' : 'bg-primary'}`} />
                  <div className="min-w-0 flex-1">
