...
import { 
  ShoppingBag, Menu, X, LogOut, ChevronLeft, ChevronRight,
  Home, Search, Package, Star, Bell, History, Settings, Bug,
  BarChart3, Users, TrendingUp, FileText, Database, AlertTriangle
} from 'lucide-react';
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-sidebar-border">
          {sidebarOpen && (
            <Link to="/" className="flex items-center gap-2">
              <div className={cn("w-9 h-9 rounded-xl bg-gradient-to-br flex items-center justify-center", roleColors[activeRole || 'user'])}>
                <ShoppingBag className="w-5 h-5 text-primary-foreground" />
              </div>
              <span className="font-display font-bold text-sidebar-foreground">MarketHub</span>
            </Link>
          )}
          <Button 
            variant="ghost" 
        ...
        <header className="h-16 bg-background/80 backdrop-blur-xl border-b border-border flex items-center px-4 lg:px-6">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm text-muted-foreground hidden sm:inline">
              Welcome back, <span className="font-medium text-foreground">{activeUserData?.name}</span>
            </span>
          </div>
        </header>
